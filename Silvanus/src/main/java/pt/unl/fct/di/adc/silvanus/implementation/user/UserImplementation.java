...
public class UserImplementation implements Users {
    ...
    //TODO TESTING
    @Override
    public Result<Void> promote(String request_user, String identifier, String new_role, String placeOfInfluence) {
        ...
        if (!result.hasNext()) {
            return Result.error(Status.NOT_FOUND, "User " + identifier + " doesn't exist");
        } else {
        ...
        if (targetRole.getPriority() > requestUserRole.getPriority()) {
            return Result.error(Status.FORBIDDEN, "Higher role than logged in user");
        }
